...
<head>
	<style>
		...
		input[type="text"] { width: 100%; max-width: 500px; padding: 4px 8px; border: 1px solid #ccc; border-radius: 4px; }
		.readonly-input[readonly] { background-color: #f0f0f0; color: #666; cursor: not-allowed; }
		button { margin-top: 16px; padding: 8px 24px; background: #0078D4; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 14px; }
		button:hover { background: #106EBE; }
		.version-badge { display: inline-block; background: #0078D4; color: white; padding: 2px 8px; border-radius: 10px; font-size: 11px; margin-left: 8px; }
	</style>
</head>
<body>
	<h2>ShardM SAP DM POD Plugin Generator <span class="version-badge">v1</span></h2>

	<h3>Select POD Version</h3>
	<div>
		<label><input type="radio" name="podVersion" value="POD 1.0" onchange="updateHostField()"> POD 1.0 (not recommended for new plugins)</label>
		<label><input type="radio" name="podVersion" value="POD 1.1" onchange="updateHostField()" checked> POD 1.1</label>
		<label><input type="radio" name="podVersion" value="POD 1.1 CLI" onchange="updateHostField()"> POD 1.1 Using UI5 CLI</label>
		<label><input type="radio" name="podVersion" value="POD 2.0 UI5" onchange="updateHostField()"> POD 2.0 Using UI5</label>
		<label><input type="radio" name="podVersion" value="POD 2.0 React" onchange="updateHostField()"> POD 2.0 Using React</label>
	</div>
	...
	<script>
		...
		function updateHostField() {
			...
			if (selected === 'POD 2.0 React' || selected === 'POD 2.0 UI5') {
				...
				languageGroup.style.display = (selected === 'POD 1.1 CLI') ? 'none' : 'block';
				npmButton.style.display = (selected === 'POD 1.1 CLI') ? 'inline' : 'none';
			}
		}
		updateHostField();
